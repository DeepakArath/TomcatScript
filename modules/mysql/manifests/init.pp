...
exec {"2":
        command => "echo percona-server-server-5.5   percona-server-server/root_password password $mysql::password | sudo debconf-set-selections",
        require => Exec ["1"], 
	 }

exec {"3":
        command => "echo percona-server-server-5.5   percona-server-server/root_password_again password $mysql::password | sudo debconf-set-selections",
        require => Exec ["2"],
     }
